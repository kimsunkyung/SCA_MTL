...
from models.seg_hrnet import BasicBlock, GlobalContextAttentionBlock
...
class MultiTaskDistillationModule(nn.Module):
    ...
    def __init__(self, tasks, auxilary_tasks, channels):
        ...
        self.N = len(auxilary_tasks)-1
        self.concat_channels = channels * self.N


        downsample = nn.Sequential(nn.Conv2d(self.concat_channels*2, channels, 1, bias=False),
                                   nn.BatchNorm2d(channels))
        self.dimensionality_reduction = BasicBlock(self.concat_channels*2, channels,
                                                   downsample=downsample)
        self.cross_attention = {}
        for t in self.tasks:
            other_tasks = [a for a in self.auxilary_tasks if a != t]
            self.cross_attention[t] = nn.ModuleDict(
                {a: GlobalContextAttentionBlock(channels, channels) for a in other_tasks})

        self.cross_attention = nn.ModuleDict(self.cross_attention)


    def forward(self, x):
        # adapters = {t: {a: self.self_attention[t][a](x['features_%s' %(a)]) for a in self.auxilary_tasks if a!= t} for t in self.tasks}
        adapters = {t: {a: torch.cat([x['features_%s' % (t)], self.cross_attention[t][a](x['features_%s' % (t)], x['features_%s' % (a)])], dim=1) for a in
                        self.auxilary_tasks if a != t} for t in self.tasks}
        c_adapters = {}
        for t in self.tasks:
            concat_ad = None
            for i in adapters[t]:
                if concat_ad == None:
                    concat_ad = adapters[t][i]
                else:
                    concat_ad = torch.cat([adapters[t][i], concat_ad], dim=1)

            c_adapters[t] = self.dimensionality_reduction(concat_ad)

        out = {t: x['features_%s' % (t)] + c_adapters[t] for t in self.tasks}
