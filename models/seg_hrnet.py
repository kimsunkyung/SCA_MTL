...
class HighResolutionHead(nn.Module):
    def __init__(self, backbone_channels, num_outputs):
        ...
        self.cross_scale_attention2 = GlobalContextAttentionBlock(backbone_channels[2], backbone_channels[2])
        self.upsample32 = nn.Sequential(nn.Conv2d(backbone_channels[3], backbone_channels[2], 1, bias=False),
                                   nn.BatchNorm2d(backbone_channels[2]))

        self.cross_scale_attention1 = GlobalContextAttentionBlock(backbone_channels[1], backbone_channels[1])
        self.upsample21 = nn.Sequential(nn.Conv2d(backbone_channels[2], backbone_channels[1], 1, bias=False),
                                       nn.BatchNorm2d(backbone_channels[1]))
        self.upsample31 = nn.Sequential(nn.Conv2d(backbone_channels[3], backbone_channels[1], 1, bias=False),
                                        nn.BatchNorm2d(backbone_channels[1]))

        self.cross_scale_attention0 = GlobalContextAttentionBlock(backbone_channels[0], backbone_channels[0])
        self.upsample10 = nn.Sequential(nn.Conv2d(backbone_channels[1], backbone_channels[0], 1, bias=False),
                                       nn.BatchNorm2d(backbone_channels[0]))
        self.upsample20 = nn.Sequential(nn.Conv2d(backbone_channels[2], backbone_channels[0], 1, bias=False),
                                        nn.BatchNorm2d(backbone_channels[0]))
        self.upsample30 = nn.Sequential(nn.Conv2d(backbone_channels[3], backbone_channels[0], 1, bias=False),
                                        nn.BatchNorm2d(backbone_channels[0]))
    ...
    def forward(self, x):
        ...
        #x2
        x32 = self.upsample32(x[3])
        x23 = self.cross_scale_attention2(x[2],x32)
        x23  = torch.cat((x[2],x23), dim = 1)
        x2 = F.interpolate(x23, (x0_h, x0_w), mode='bilinear')

        #x1
        x21 = self.upsample21(x[2])
        x12 = self.cross_scale_attention1(x[1],x21)
        x12 = torch.cat((x[1],x12),dim = 1)
        x1 = F.interpolate(x12, (x0_h, x0_w), mode='bilinear')

        x31 = self.upsample31(x[3])
        x13 = self.cross_scale_attention1(x[1], x31)
        x13 = torch.cat((x[1], x13), dim=1)
        x11 = F.interpolate(x13, (x0_h, x0_w), mode='bilinear')

        #x0
        x10 = self.upsample10(x[1])
        x01 = self.cross_scale_attention0(x[0],x10)
        x0 = torch.cat([x[0],x01],dim=1)

        x20 = self.upsample20(x[2])
        x02 = self.cross_scale_attention0(x[0], x20)
        x00 = torch.cat([x[0], x02], dim=1)

        x30 = self.upsample30(x[3])
        x03 = self.cross_scale_attention0(x[0], x30)
        x000 = torch.cat([x[0], x03], dim=1)

        x = torch.cat([x0, x00, x000, x1, x11, x2, x3], 1)
        x = self.last_layer(x)
        return x
